...
{
    class SchemaImageClassification
    {
        ...
        */
        static List<int> ReadCsvFileTest(String path)
        {
        ...
        */
        static void saveOutput(List<int[]> activeArray, String path, string[] imageNames)
        {
        ...
        public static string binarizeImage(string sourcepath, int imageWidth, int imageHeight, string destinationPath, string name)
        {
            string binaryImage;
        ...
        static void writeFile(string sourcePath, string destinationPath, string fileName, string foldername)
        {
        ...
        static Parameters GetDefaultParams()
        {
        ...
        static Parameters getParam(int inputDimension, int activeColumn)
        {
        ...
        static (string, string) getPathAndName(string fullPath)
        {
        ...
        static string groupping(List<double[]> activeArray, string[] imageNames, string path)
        {
        ...
        static string report(List<double[]> distance, string[] names,string outputFilePath,string outputFileName)
        {
            if (Directory.Exists(outputFilePath)) DeleteDirectory(outputFilePath);
            else Directory.CreateDirectory(outputFilePath);
            string filePath = Path.Combine(outputFilePath,outputFileName);//"..\\..\\..\\distance.xlsx";
            if (File.Exists(filePath)) File.Delete(filePath);

            WorkBook workbook = WorkBook.Create();
            WorkSheet sheet = workbook.CreateWorkSheet(outputFileName);
        ...
        public static string RunSEProject(string inputFolder,string outputFolder, string outputFileName)
        {

            Console.WriteLine("in run");
            Console.WriteLine(Directory.GetCurrentDirectory());
            var imagesFolder = inputFolder;
            var csvPath = Path.Combine(inputFolder,"..","csv");
            if (Directory.Exists(csvPath)) DeleteDirectory(csvPath);
            else Directory.CreateDirectory(csvPath);

            string[] files = Directory.GetFiles(imagesFolder, "*", SearchOption.AllDirectories);

            string path, name; (path, name) = getPathAndName(files[0]);
            ...
            string groupImagePath = groupping(distance, imageNames, path);
            var resultXLPath = Path.Combine(inputFolder, "..", outputFolder);
            report(distance, imageNames,resultXLPath,outputFileName);
